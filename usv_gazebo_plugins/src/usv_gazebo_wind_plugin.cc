...
void UsvWindPlugin::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  {
    windCoeff = _sdf->GetElement("wind_coeff_vector")->Get<ignition::math::Vector3d>();
  }
...
void UsvWindPlugin::Update()
{
  ...
  ignition::math::Vector3d relativeWind = this->link->WorldPose().Rot().Inverse().
  ...
  this->link->AddRelativeForce(ignition::math::Vector3d(windForce.X(), windForce.Y(), 0.0));
  this->link->AddRelativeTorque(ignition::math::Vector3d(0.0, 0.0, windForce.Z()));
}
